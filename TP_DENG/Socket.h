...
public:
   ...
   * @see SocketBuffer to preserve record boundaries.
   * @see the send() system call for low level details.
   */
   ...
   * except if end-of-stream is reached (shutdownOutput() was called on the other 
   * side) or an error occurs.
   * @return the number of bytes received, 0 at end-of-stream, -1 in case of an error.
   * @note that that connected sockets do not preserve record boundaries (@see SocketBuffer).
   * @see SocketBuffer to preserve record boundaries.
   * @see the recv() system call for low level details.
   */
   ...
   * @return the number of bytes sent or -1 in case of an error.
   * @see the sendto() system call for low level details.
   */
   ...
   * @see the recvfrom() system call for low level details.
   */
...
public:
  /** constructor.
   * The argument must be a valid connected (SOCK_STREAM) Socket which must not
   * be destructed while the SocketBuffer object is used.
   */
  SocketBuffer(Socket*);
  SocketBuffer(Socket&);
  ...
  /** Sends a given number of bytes to a connected socket.
   * Writes *exactly* _len_ bytes to the socket (in constrast with Socket::send()
   * which may not send all bytes) except if an error occurs. 
   * Practically, this function calls Socket::send() several times if needed.
   * @return the number of bytes sent or -1 in case of an error
   * @see Socket::send() for more details.
   */
  virtual ssize_t write(const void* buf, size_t len);
  
  /** Receives a given number of bytes from a connected socket.
   * Reads *exactly* _len_ bytes from the socket (in constrast with Socket::receive()
   * that reads *at most* _len_ bytes) except if end-of-stream is reached (shutdownOutput()
   * was called on the other side) or an error occurs.
   * Practically, this function calls Socket::receive() several times if needed.
   * @return the number of bytes received, 0 at end-of-stream, -1 in case of an error.
   ...
   * Reads characters from the socket and stores them into _str_ until a newline
   * is read (character '\n' or '\r'), end-of-stream is reached (shutdownOutput()
   * was called on the other side) or an error occurs.
   * Practically, this function calls Socket::receive() several times if needed.
   * @return the number of bytes received (including the newline), 0 at end-of-stream,
  ...
  /** Reads a line of text from a connected socket.
   * Reads characters from the socket and stores them into _str_ until: a) len-1
   * bytes are read, b) a newline is read (character '\n' or '\r'), c) end-of-stream
   * is reached (shutdownOutput() was called on the other side), d) an error occurs.
   * Practically, this function calls Socket::receive() several times if needed.
   * @return the number of bytes received (including the newline), 0 at end-of-stream,
   * -1 in case of an error.
   * _truncated_ is true if the function receives more than len-1 bytes. The remaining
   * bytes can be read by calling this function again. _str_ is always nul terminated.
